namespace Task2 {
  function processInput(inputStr: string): number {
    ...
    let currentGuardSleepEnd: number;

    lineArraySorted.forEach(line => {
      if (line.indexOf("Guard") > -1) {
        currentGuardId = parseInt(
          line.slice(line.indexOf("#") + 1, line.indexOf(" begins"))
        );
    ...
    let id: number;
    let highestMinuteAsleep: number = 0;
    let highestMinuteAsleepCount: number = 0;
